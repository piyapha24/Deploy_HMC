...
$(document).ready(function () {
    ...
    connection.start().then(() => {
        ...
        connection.on("ReceiveScrapListInsert", function (newData) {
            ...
            tr.append('<td class="text-left text-success">' + formattedDate + '</td>'); // ใช้วันที่ที่แปลงแล้ว
            tr.append('<td class="text-left text-warning"> Waiting </td>');
            tr.append('<td class="text-left text-warning"> Waiting </td>');
            tr.append('<td class="text-left text-warning"> Waiting </td>');
        ...
        connection.on("ReceiveScrapListEdit", function (updatedData) {
            ...
            if (row.length) {
                ...
                var formattedDate = updatedData.contractExpireDate ?
                ...
                row.data([
                    ...
                    "<span class='text-warning'>Waiting</span>",  // Column 3: Status or remaining
                    formattedDate,                        // Contract Expire Date (with text-success)
                    "<span class='text-warning'>Waiting</span>",  // Column 5: Industry Expiry Date
                    "<span class='text-warning'>Waiting</span>",  // Column 6: Transfer Quantity
                    "<span class='text-warning'>Waiting</span>",  // Column 7: Department Name
