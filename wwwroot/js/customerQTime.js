function DocNoList() {
    let formData = new FormData($('#frmQTimes')[0]);
    $.ajax({
        type: 'POST',
        url: routeDocNo,
        data: formData,
                ...
                dayCellContent: function (info) {
                    ...
                    if (session1Data) {
                        ...
                        buttonMorning = `
                            <button class="time-btn" data-time="afternoon" data-date="${info.date}"
                                style="background-color: #FF6962; border-radius: 5px; color: white; margin-left: 10px; padding: 15px;">
                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-sunrise block mx-auto">
                                    <path d="M12 2v8"></path>
                                    <path d="M5.2 11.2l1.4 1.4"></path>
                                    <path d="M2 18h2"></path>
                    ...
                    var customHtml = `
                     <div class="d-flex flex-column align-items-center" cursor: pointer;">
                       <a href="/WEB/Customer/QtimeToday?datefor=${formatDate}">
...
function goToDriverConfirm(id) {
    $.ajax({
        url: "/WEB/Customer/DriverConfirms/" + id,
        type: 'POST',
        dataType: "json",
        success: function (response) {
...
function submitChangeDeliveryForm() {
    ...
    var changeMoving = $('input[name="answer"]:checked').val(); // ค่าจาก radio button ที่ถูกเลือก
    var lockMoving = $('input[name="answer"]:checked').val() === 'false'; // ค่าจาก radio button ที่ถูกเลือก
    ...
    formData.push({ name: 'changeMoving', value: changeMoving });
    formData.push({ name: 'lockMoving', value: lockMoving });

    // Send AJAX request
    $.ajax({
        url: '/WEB/Customer/ChangeDelivery',
        type: 'POST',
        data: $.param(formData), // Converts array back to a URL-encoded string
        success: function (response) {
            window.location.href = response.redirectToUrl;
